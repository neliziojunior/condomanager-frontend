...
import {
  ...
  AccountBalance // ✅ NOVO: AccountBalance
...
export default function Layout() {
  ...
  const menuItems = [
    ...
    { text: 'Contabilidade', icon: <AccountBalance />, path: '/accounting' }, // ✅ NOVO: Contabilidade
  ...
  const drawerContent = (
    <Box sx={{ 
      ...
      <Box sx={{ p: 2.5, display: 'flex', alignItems: 'center', gap: 1.5 }}>
        <Box sx={{ bgcolor: '#00A896', borderRadius: 2, width: 38, height: 38, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, color: 'white', flexShrink: 0 }}>
          🏢
        </Box>
        <Box>
          <Typography sx={{ fontWeight: 700, fontSize: 16, color: '#1A1A2E', lineHeight: 1.2 }}>CondoManager</Typography>
          <Typography sx={{ fontSize: 10, color: '#00A896', fontWeight: 600, letterSpacing: 0.5 }}>GESTÃO INTELIGENTE</Typography>
        </Box>
      </Box>
        ...
        {menuItems.map((item) => {
          ...
          return (
            <ListItemButton
              ...
              sx={{
                borderRadius: 2, mb: 0.2, minHeight: 44, px: 2,
                bgcolor: isActive ? '#F0FDF9' : 'transparent',
                color: isActive ? '#00A896' : '#374151',
                '&:hover': { bgcolor: isActive ? '#F0FDF9' : '#F7F9FC', color: '#00A896' },
                transition: 'all 0.15s ease',
              }}
            >
              <ListItemIcon sx={{ minWidth: 0, mr: 2, color: isActive ? '#00A896' : '#6B7280' }}>
                {item.icon}
              </ListItemIcon>
              <ListItemText primary={item.text} primaryTypographyProps={{ fontSize: 13.5, fontWeight: isActive ? 600 : 500, color: isActive ? '#00A896' : '#374151' }} />
              {isActive && <Box sx={{ width: 3, height: 22, borderRadius: 2, bgcolor: '#00A896' }} />}
            </ListItemButton>
          );
        })}
      </List>
      <Box sx={{ p: 2, borderTop: '1px solid #F0F0F0' }}>
        ...
        <Button fullWidth onClick={() => { logout(); navigate('/login'); }} startIcon={<ExitToApp />}
          sx={{ color: '#6B7280', fontSize: 12, textTransform: 'none', borderRadius: 2, py: 1, '&:hover': { bgcolor: '#FFF5F5', color: '#E63946' } }}>
          Sair da conta
        </Button>
      </Box>
    </Box>
